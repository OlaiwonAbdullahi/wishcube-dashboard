      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:items-center lg:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm font-medium text-neutral-500 uppercase tracking-wider">
            Deep dive into your store&apos;s performance metrics and customer trends.
          </p>
        </div>
      </div>

      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 border-2 border-[#191A23] rounded-none shadow-[6px_6px_0px_0px_rgba(25,26,35,1)]">
          ...
          <CardContent className="p-6 h-[400px]">
             {loading ? (
               <div className="h-full flex items-center justify-center font-black uppercase text-xs">Loading analytics...</div>
             ) : (
        ...
        <Card className="border-2 border-[#191A23] rounded-none shadow-[6px_6px_0px_0px_rgba(25,26,35,1)]">
          <CardHeader className="border-b-2 border-[#191A23] bg-[#F3F3F3]">
            <CardTitle className="text-sm font-black uppercase tracking-wider flex items-center gap-2">
          ...
          <CardContent className="p-6 h-[400px]">
            {loading ? (
               <div className="h-full flex items-center justify-center font-black uppercase text-xs">Loading stats...</div>
             ) : (
          ...
          <CardContent className="p-6">
            <div className="space-y-6">
              <SummaryItem 
                ...
                value={`NGN ${(analyticsData?.revenueHistory.reduce((acc, curr) => acc + curr.revenue, 0) || 0) / (analyticsData?.revenueHistory.length || 1)}`.split('.')[0]} 
              />
              <SummaryItem 
                label="Total Orders Handled" 
                value={analyticsData?.ordersByStatus.reduce((acc, curr) => acc + curr.count, 0).toString() || "0"} 
              />
              <SummaryItem 
...
function SummaryItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between items-center bg-neutral-50 border-2 border-[#191A23] p-4">
      <span className="text-[10px] font-black uppercase text-neutral-400">{label}</span>
